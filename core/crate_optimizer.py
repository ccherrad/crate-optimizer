...
class CrateOptimizer:
    ...
    def __init__(
        ...
        if not all(order_product.orderId == order_products[0].orderId for order_product in order_products):
    ...
    def is_kept_in_crate(self, order_product):
        ...
        if (
            ...
            print(order_product)
            self.crate_counts["Supplier"] += order_product.unitCount // product_info["packing"]
    ...
    def calculate_optimal_crates(self):
        needed_slots = 0
        self.order_products = list(map(lambda product: self.is_kept_in_crate(product), self.order_products))

        small_needed_slots = 0
        small_product_orders = list(filter(lambda product: self.is_small_order_product(product), self.order_products))

        small_needed_slots += sum(order_product.unitCount for order_product in small_product_orders)
        self.crate_counts["Slot20"] = (small_needed_slots + 1) // 20
        if ((small_needed_slots) % 20):
            self.crate_counts["Slot20"] += 1
        

        orangina_product_orders = list(filter(lambda product: self.is_orangina(product), self.order_products))
        needed_slots += (sum(order_product.unitCount for order_product in orangina_product_orders) + 1) // 2
        
        latournee_product_orders = list(filter(lambda product: self.is_latournee_order_product(product), self.order_products))
        needed_slots +=  math.ceil( (sum(order_product.unitCount for order_product in latournee_product_orders)) * 1.2)

        other_product_orders = list(filter(lambda product: self.is_other_order_product(product), self.order_products))
        needed_slots +=  sum(order_product.unitCount for order_product in other_product_orders)
